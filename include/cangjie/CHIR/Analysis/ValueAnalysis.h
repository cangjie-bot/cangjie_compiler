...
private:
    ...
    void LoadGVChildrenState(AbstractObject* src, LocalVar* dest)
    {
        ...
        const auto setChildState = [this, &objChildren](AbstractObject* child, size_t index) {
            auto& srcState = ValueAnalysis<ValueDomain, ValueStatePool>::globalState.programState.At(objChildren[index]);
...
private:
    ...
    template <typename TAllocate> Value* PreHandleAllocateExpr(State<ValueDomain, ValueStatePool>& state, const TAllocate* allocate)
    {
    ...
    template <typename TApply> std::optional<Block*> PreHandleApplyExpr(State<ValueDomain, ValueStatePool>& state, const TApply* apply)
    {
    ...
    template <typename T> Value* PreHandleFuncCall(State<ValueDomain, ValueStatePool>& state, const T* apply)
    {
    ...
    static void SetObjChildrenStateToTop(State<ValueDomain, ValueStatePool>& state, Value* root, Type* rootTy, CHIRBuilder& builder)
    {
    ...
    std::optional<Block*> PreHandleRawArrayAllocate(State<ValueDomain, ValueStatePool>& state, const TRawArrayAllocate* allocate)
    {
    ...
    std::optional<Block*> PreHandleInoutIntrinsic(State<ValueDomain, ValueStatePool>& state, const TIntrinsic* intrinsic)
    {
    ...
    virtual void HandleAllocateExpr(State<ValueDomain, ValueStatePool>& state, const Allocate* expression, Value* refObj)
    {
        (void)state;
        (void)expression;
        (void)refObj;
    ...
    virtual std::optional<Block*> HandleTerminatorEffect(State<ValueDomain, ValueStatePool>& state, const Terminator* terminator)
    {
        (void)state;
        (void)terminator;
        return std::nullopt;
