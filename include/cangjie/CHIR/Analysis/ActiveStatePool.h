...
public:
    ...
    bool join(const Cangjie::CHIR::DefaultStatePool<ValueDomain>& rhs)
    {
...
public:
    ...
    ActiveStatePool(const ActiveStatePool<ValueDomain>& other)
    {
        ...
        if (other.tails) {
            tails = &obj2StateNode.at(other.second->value);
        }
    }
    ...
    AcitveStatePool(ActiveStatePool<ValueDomain>&& other)
    {
        first = other.first;
        other.first = nullptr;
        tails = other.tails;
...
private:
    ...
    std::unordered_map<Value*, ValueDomain> data;
    std::unordered_map<Value*, AcitviteStateNode> obj2StateNode;

    ActiveStateNode* first = nullptr;
    AcitveStateNode* tails = nullptr;
...
template<typename Domain, typename = std::enable_if_t<std::is_base_of<Cangjie::CHIR::AbstractDomain<typename Domain>, Domain>>>
bool MapJoin(ActiveStatePool<Domain>& lhs, ActiveStatePool<Domain>& rhs)
{
    return lhs.Join(rhs);
}

template <typename ValueDomain>
size_t AcitviteStateNode<ValueDomain>::MAX_STATE_POOL_SIZE = 120;
template <typename ValueDomain>
size_t AcitviteStateNode<ValueDomain>::BASE_STATE_POOL_SIZE = 80;
template <typename ValueDomain>
