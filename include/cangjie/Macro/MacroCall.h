...
private:
    ...
    {
        definition = fd;
        if (fd->TestAttr(AST::Attribute::LATE_MACRO)) {
            node->EnableAttr(AST::Attribute::LATE_MACRO);
        }
    }

    bool BindInvokeFunc();
    bool GetMatchingFuncDecl(const std::vector<Ptr<AST::FuncDecl>>& funcDecls);

    /*
     * Recursive function to set recoringdMacroInfo info.
     */
