...
void ParserImpl::AttachCommentToSortedNodes(std::vector<Ptr<Node>>& nodes, CommentGroupsLocInfo& cgInfo)
{
...
void ParserImpl::AttachCommentToNodes(std::vector<OwnedPtr<Node>>& nodes)
{
    ...
    AttachCommentToSortedNodes(nps, cgInfo);
}

void ParserImpl::CollectFileCG(const std::vector<Ptr<AST::Node>>& nodes, CommentGroupsLocInfo& cgInfo, Ptr<File> node)
{
    size_t begin{0};
    ...
    cgInfo.commentGroups.erase(cgInfo.commentGroups.begin(), cgInfo.commentGroups.begin() + begin);
    if (cgInfo.commentGroups.empty()) {
        return;
    }
    begin = cgInfo.commentGroups.size() - 1;
    auto endLine{0};
    for (auto& n: nodes) {
        endLine = std::max(endLine, n->end.line + 1);
    }
    while (true) {
        if (cgInfo.commentGroups[begin].cms.back().info.Begin().line > endLine) {
            AddTrailingComment(*node, cgInfo.commentGroups[begin]);
            if (begin-- == 0) {
                break;
            }
        } else {
    ...
    cgInfo.commentGroups.erase(cgInfo.commentGroups.begin() + begin + 1, cgInfo.commentGroups.end());
}
...
void ParserImpl::AttachCommentToFile(Ptr<File> node)
{
    ...
    CommentGroupsLocInfo cgInfo{{}, {}, {}, lexer->GetTokenStream()};
    CollectCommentGroups(cgInfo);
    CollectFileCG(nodes, cgInfo, node);
    AttachCommentToSortedNodes(nodes, cgInfo);
}