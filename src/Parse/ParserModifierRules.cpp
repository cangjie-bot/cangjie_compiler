...
// declare conflict rule with an array literal, represented by var args
...
// declare conflict rule with named array
#define CR_ARR(tk, arr) {tk, arr, ARRAY_SIZE(arr)}

// Shared conflict arrays (reused in multiple rules, accessed at runtime via pointers)
...
std::optional<AST::Attribute> GetAttributeByModifier(TokenKind tokenKind)
{
    ...
    int lo = 0, hi = static_cast<int>(ARRAY_SIZE(TABLE)) - 1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        if (TABLE[mid].tk == tokenKind)
            return TABLE[mid].attr;
        if (static_cast<int>(TABLE[mid].tk) < static_cast<int>(tokenKind)) {
            lo = mid + 1;
        } else {
            hi = mid - 1;
        }
