...
Token LexerImpl::ScanIntegerOnlyNumber(
    const char* pStart, const char* reasonPoint, char prefix, DigitScanner scanDigits)
{
    ...
    (this->*scanDigits)(hasDigit, reasonPoint);
    if (!hasDigit && success) {
        DiagExpectedDigit(prefix);
        success = false;
    }
...
{
    return ScanIntegerOnlyNumber(pStart, reasonPoint, 'b', &LexerImpl::ScanBinDigits);
}

// Scan octal number: '0' O [0-7] ([0-7] | '_')* IntSuffix?
Token LexerImpl::ScanOctNumber(const char* pStart, const char* reasonPoint)
{
    return ScanIntegerOnlyNumber(pStart, reasonPoint, 'o', &LexerImpl::ScanOctDigits);
}
