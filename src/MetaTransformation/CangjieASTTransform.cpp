...
#include "cangjie/MetaTransformation/ASTMetaTransform.h"
...
const char* SerialiseOptions(const GlobalOptions& opts)
{
    std::vector<char> buf;
    buf.reserve(4096);
...
{
    auto executeASTPlugins = reinterpret_cast<bool (*)(const void*, int32_t, const char*)>(FindSymbolInStdLibs(
    ...
    const char* options = SerialiseOptions(ci.invocation.globalOptions);
    // must pass integer, because cangjie enum is not expressible in C++
    auto res = executeASTPlugins(&package, stage == CompileStage::MACRO_EXPAND ? 0 : 1, options);
    delete[] options;
    return res;
}
