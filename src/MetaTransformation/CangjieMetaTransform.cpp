...
Memory RunPlugin(Memory pkg, std::string_view pluginPath, CompilerInvocation& invoc)
{
    ...
    return Memory{nullptr, 3}; // 3: non-Linux platform
#else
    ...
    if (mem == MAP_FAILED) {
        ...
        return Memory{nullptr, 2}; // 2: resource failure (mmap)
    }
    ...
    if (pipe(ctrlPipe) != 0) {
        ...
        return Memory{nullptr, 2}; // 2: resource failure (pipe)
    }
    ...
    if (shmFd < 0) {
        ...
        close(ctrlPipe[1]);
        munmap(mem, totalSize);
        return Memory{nullptr, 2}; // 2: resource failure (shm_open)
    }
    ...
    if (pid < 0) {
        ...
        close(ctrlPipe[1]);
        munmap(mem, totalSize);
        return Memory{nullptr, 2}; // 2: resource failure (fork)
    }
    if (pid == 0) {
        ...
        if (!runtimeInitSuccess) {
            ...
            size_t fail = 0;
            write(ctrlPipe[1], &fail, sizeof(size_t));
            _exit(2); // 2: resource failure
        }
        ...
        if (!pluginHandle) {
            ...
            size_t fail = 0;
            write(ctrlPipe[1], &fail, sizeof(size_t));
            _exit(1); // 1: child/plugin failure
        }
        ...
        if (size == 0 || outPkg == nullptr) {
            size_t zero = 0;
            write(ctrlPipe[1], &zero, sizeof(size_t));
            _exit(0);
        }
        ...
        write(ctrlPipe[1], &size, sizeof(size_t));
    ...
    if (nread != static_cast<ssize_t>(sizeof(size_t)) || outSize == 0) {
        ...
        size_t ec = 1; // default to child failure
        if (WIFEXITED(status)) {
            int code = WEXITSTATUS(status);
            if (code == 2) {
                ec = 2; // resource failure in child
            } else if (code == 0) {
                ec = 1; // zero-size output but child exited 0 -> treat as failure
            } else {
                ec = 1; // any non-zero -> child failure
            }
        }
        close(shmFd);
        shm_unlink(shmName);
    ...
    if (inMap == MAP_FAILED) {
        ...
        shm_unlink(shmName);
        munmap(mem, totalSize);
        return Memory{nullptr, 2}; // 2: resource failure (parent mmap)
    }
    ...
    if (!outBuf) {
        ...
        shm_unlink(shmName);
        munmap(mem, totalSize);
        return Memory{nullptr, 2}; // 2: resource failure (OOM)
    }
