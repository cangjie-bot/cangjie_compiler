...
public:
    ...
    Package* Deserialize()
    {
        ...
        for (auto& v: parsedGVs) {
            auto gv = b.CreateGlobalVar(INVALID_LOCATION, StaticCast<RefType>(ToCHIRType(v)), v.identifier, v.identifier,
                v.identifier, pkg->GetName());
            auto init = b.CreateLiteralValue<IntLiteral>(GetInitializerType(v), v.value);
            gv->SetInitializer(*init);
            gv->Dump();
        }
        // std::cout << "in C++ package: " << pkg->ToString();
        return pkg;
    }
...
{
    auto [pkg, sz] = CHIRSerializer::Serialize(p, ToCHIR::Phase::RAW);
    ...
    auto tmp = new(std::nothrow) Memory{pkg, sz};
    if (!tmp) {
        Errorln("Failed to allocate memory in meta transform");
        return nullptr;
    }
    // std::cout << "C++ input pointer: " << tmp->ptr << ", size: " << tmp->size << "\n";
    invokeC(fn, tmp);
    // auto phase = ToCHIR::Phase::RAW;
    CHIRBuilder b{ctx};
    // CHIRDeserializer::Deserialize(tmp->ptr, tmp->size, b, phase);
    ...
    // free(pkg);
    if (pkg != tmp->ptr) {
        // printf("to delete tmp->ptr %p\n", tmp->ptr);
        free(tmp->ptr);
    }
    // printf("to delete tmp struct %p\n", tmp);
    free(tmp);
    
    std::this_thread::sleep_for(std::chrono::milliseconds(100)); // wait for cangjie to print
    return ret;
}
}
...
bool CompilerInstance::ExecuteCHIRPlugins()
{
    ...
    for (auto &plugin : cangjieCHIRPlugins) {
        pkg = Run(chirData.GetCHIRContext(), *pkg, plugin);
        if (!pkg) {
            Errorln("run CHIR plugin returned null package");
            return false;
        }
    }
    chirData.SetPackage(*pkg);
    std::abort();
    return true;
}
}