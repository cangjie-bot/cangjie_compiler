...
OwnedPtr<Type> WrapType(Ptr<Ty> ty)
{
    if (!ty || !Ty::IsTyCorrect(ty) || ty->IsInvalid()) {
        return nullptr;
    }
...
void ASTLoader::ASTLoaderImpl::SubstituteImportedTypeAliasTy()
{
    ...
    Walker(curPackage, [this](Ptr<Node> node) {
    ...
    Println("SubstituteImportedTypeAliasTy", curPackage->fullPackageName, "spend:", duration, "ms");
}
