...
void ASTWriter::ASTWriterImpl::MarkImplicitExportOfImportSpec(Package& package)
{
    for (auto& file : package.files) {
        for (auto& import : file->imports) {
            ...
            auto importPkgName = Modules::GetImportPackageNameByImportSpec(*import);
            if (!Utils::In(importPkgName, implicitPackageNames) && !import->IsReExport(package.noSubPkg)) {
                import->withImplicitExport = false;
            } else {
                implicitPackageNames.erase(importPkgName);
            }
            Debugln("++++", importPkgName, import->content.ToString(), "mark as", import->withImplicitExport);
        }
    }
    for (auto implicitAddedPkgName : implicitPackageNames) {
        auto implicitAdd = CreateImportSpec(implicitAddedPkgName);
        package.files[0]->imports.emplace_back(std::move(implicitAdd));
        Debugln("implicit add import", implicitAddedPkgName);
    }
    for (auto& file : package.files) {
...
void ASTWriter::ASTWriterImpl::AST2FB(std::vector<uint8_t>& data, const PackageDecl& package)
{
    ...
    for (auto& depStdPkg : package.srcPackage->allDependentStdPkgs) {
...
TFullIdOffset ASTWriter::ASTWriterImpl::GetFullDeclIndex(Ptr<const Decl> decl)
{
    ...
    if (decl->astKind == ASTKind::PACKAGE_DECL) {
        ...
        Debugln("astwriter need decl", decl->fullPackageName, decl->identifier.Val());
        implicitPackageNames.emplace(decl->fullPackageName);
