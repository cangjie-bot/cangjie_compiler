...
bool ImportManager::ResolveImportedPackageForFile(File& file, bool isRecursive)
{
    ...
    for (auto& import : file.imports) {
        ...
        if (importPkg != nullptr) {
            CJC_ASSERT(!importPkg->TestAttr(Attribute::IMPORTED) ||
                        importPkg->TestAttr(Attribute::TOOL_ADD) ||
                        !cjoPath.empty());
            if (!importPkg->TestAttr(Attribute::IMPORTED) && curPkg->TestAttr(Attribute::IMPORTED)) {
                ...
                success =
                    HandleParsedPackage(*importPkg, cjoPath, !isMacroRelated || isVisible, isRecursive) && success;
            }
            continue;
        }
