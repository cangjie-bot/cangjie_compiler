...
bool ImportManager::ResolveImportedPackageForFile(File& file, bool isRecursive)
{
    ...
    for (auto& import : file.imports) {
        if (import->IsImportMulti() || !import->withImplicitExport) {
            if (!import->withImplicitExport)
                Debugln("skip", file.GetFullPackageName(), "import at", import->content.ToString());
            continue;
        }
        ...
        Debugln("resolve", file.GetFullPackageName(), "import at", import->content.ToString());
        ...
        if ((!IsMacroRelatedPackageName(file.curPackage->fullPackageName) && !package->isMacroPackage) ||
            ...
            for (auto depStdPkg : package->allDependentStdPkgs) {
                auto depCjoPath =
                    FileUtil::FindSerializationFile(depStdPkg, SERIALIZED_FILE_EXTENSION, GetSearchPath());
...
bool ImportManager::ResolveImportedPackages(const std::vector<Ptr<Package>>& packages)
{
    ...
    for (auto pkg : packages) {
        ...
        for (auto [_, typeWithFullPkgName] : stdDepsMap) {
            Debugln(curPackage->fullPackageName, "record dependency", typeWithFullPkgName.second);
            curPackage->allDependentStdPkgs.emplace_back(typeWithFullPkgName.second);
        }
        Println("loaded packages size:", cjoManager->GetLoadedPackageSize());
    }
