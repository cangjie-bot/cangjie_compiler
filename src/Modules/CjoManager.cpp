...
{
    if (!impl->GetGlobalOptions().IsCompilingCJMPPlatform()) {
        return;
    }
...
//
//     if (globalOptions.commonPartCjo.size() < 0) {
...
std::vector<OwnedPtr<ASTLoader>>& CjoManagerImpl::GetCommonPartCjos(std::string packageName, const CjoManager& cjoManager)
{
