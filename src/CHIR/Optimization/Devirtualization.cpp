...
void Devirtualization::RunOnFunc(const Func* func, CHIRBuilder& builder)
{
    ...
    std::pair<bool, std::unordered_set<GenericType*>> visibleGenericTypes = {false, {}};
    const auto actionBeforeVisitExpr =
        ...
        if (thisType->IsGenericRelated()) {
            if (!visibleGenericTypes.first) {
                auto types = GetVisiableGenericTypes(*func);
                visibleGenericTypes.first = true;
                visibleGenericTypes.second = std::unordered_set<GenericType*>(types.begin(), types.end());
            }
            /*
             ...
             */
            for (auto it : thisType->GetTypeArgs()) {
                if (it->IsGeneric() && visibleGenericTypes.second.count(StaticCast<GenericType*>(it)) == 0) {
                    return;
                }
            }
        }
