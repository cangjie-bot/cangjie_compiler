...
{
    for (const auto& vtableIt : vtables) {
...
void VTableInDef::AddNewItemToTypeVTable(ClassType& srcParent, VirtualMethodInfo&& funcInfo)
{
    for (auto& vtableIt : vtables) {
        if (vtableIt.TheFirstLevelIsMatched(srcParent)) {
            vtableIt.AppendNewMethod(std::forward<VirtualMethodInfo>(funcInfo));
            return;
        }
    }
...
{
    for (auto& vtableIt : vtables) {
        CJC_ASSERT(!vtableIt.TheFirstLevelIsMatched(srcParent));
    }
