...
bool IsOptionalCType(const Type& type, bool needToBeCStruct, bool CStringCount)
{
    if (auto varray = Cangjie::DynamicCast<const VArrayType*>(&type)) {
        return IsOptionalCType(*varray->GetElementType(), needToBeCStruct, CStringCount);
    } else if (auto tuple = Cangjie::DynamicCast<const TupleType*>(&type)) {
        for (auto t : tuple->GetElementTypes()) {
            if (!IsOptionalCType(*t, needToBeCStruct, CStringCount)) {
                return false;
            }
        }
        return true;
    } else if (auto ref = Cangjie::DynamicCast<const RefType*>(&type)) {
        return IsOptionalCType(*ref->GetBaseType(), needToBeCStruct, CStringCount);
    } else if (auto cp = Cangjie::DynamicCast<const CPointerType*>(&type)) {
        return IsOptionalCType(*cp->GetElementType(), needToBeCStruct, CStringCount);
    } else if (auto cf = Cangjie::DynamicCast<const FuncType*>(&type); cf && cf->IsCFunc()) {
        if (!IsOptionalCType(*cf->GetReturnType(), needToBeCStruct, CStringCount)) {
            return false;
        }
        ...
        return true;
    } else if (CStringCount && type.IsCString()) {
        return true;
    }
    auto k = type.GetTypeKind();
    return (k >= Type::TypeKind::TYPE_INT8 && k <= Type::TypeKind::TYPE_NOTHING) ||
        type.IsCPointer() || IsStructOrCStruct(type, needToBeCStruct);
}
...
bool IsCTypeIncludeNormalStruct(const Type& type)
{
    return IsOptionalCType(type, false, true);
}

bool IsCTypeExcludeCString(const Type& type)
{
    return IsOptionalCType(type, false, false);
}
    ...
    if (auto localVar = DynamicCast<const LocalVar*>(&op)) {
        ...
        if (auto load = DynamicCast<const Load*>(localExpr)) {
            ...
            for (auto p : ger->GetPath()) {
                ...
                if (!IsCTypeExcludeCString(*locationType)) {
...
void CHIRChecker::CheckInout(const IntrinsicBase& expr, const Func& topLevelFunc)
{
    ...
    if (!IsCTypeExcludeCString(*resultType)) {
...
void CHIRChecker::CheckVArray(const VArray& expr, const Func& topLevelFunc)
{
    ...
    if (!IsCTypeIncludeNormalStruct(*elemType)) {
