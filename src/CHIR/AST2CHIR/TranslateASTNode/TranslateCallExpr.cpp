...
void Translator::BlackBoxModifyArgTypeToRef(std::vector<Value*>& args, Type* retTy)
{
    ...
    for (auto& arg : args) {
        ...
        if (arg->IsLocalVar()) {
            ...
            if (expr->IsLoad()) {
                ...
                newArg = TryCreate<Allocate>(currentBlock, loc, retTy, type)->GetResult();
...
Ptr<Value> Translator::TranslateIntrinsicCall(const AST::CallExpr& expr)
{
    ...
    if (intrinsicKind == BLACK_BOX) {
        BlackBoxModifyArgTypeToRef(args, ty);
