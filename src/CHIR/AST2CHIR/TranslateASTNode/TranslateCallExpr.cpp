...
Value* Translator::TranslateMemberFuncCall(const AST::CallExpr& expr)
{
    ...
    if (!calleeIsStatic) {
        ...
        CJC_ASSERT(!instCallInfo.instParamTypes.empty());
        thisObj = TypeCastOrBoxIfNeeded(*thisObj, *instCallInfo.instParamTypes[0], loc);
        args.emplace_back(thisObj);
    }
    auto paramInstTysWithoutThisTy = instCallInfo.instParamTypes;
    if (!calleeIsStatic) {
