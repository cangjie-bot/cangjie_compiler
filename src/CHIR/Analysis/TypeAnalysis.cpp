...
bool TypeAnalysis::Filter(const Func& method)
{
    ...
    if (method.GetPackageName().substr(0, strlen(STD_PACKAGE_PREFIX)) == STD_PACKAGE_PREFIX) {
        return true;
    }
    const static size_t OVERHEAD_BLOCK_SIZE = 300U;
    return GetAllBlockSizeOfFunc(method, OVERHEAD_BLOCK_SIZE) < OVERHEAD_BLOCK_SIZE;
}
