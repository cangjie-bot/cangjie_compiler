...
void TypeAnalysis::PreHandleMemberAccess(TypeDomain& state, const TMemberAccess* memberAccess)
{
    ...
    auto result = it2->second->StripAllRefs();
    auto instanceType = GetInstSubType(*result, *memberAccess->GetResult()->GetType()->StripAllRefs(), builder);
...
void PreHandleGetElementRefExpr(TypeDomain& state, const GetElementRef* getElemRef)
{
