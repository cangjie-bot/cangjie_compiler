    ...
    const CHIR::EnumType& enumTy, CodeGenDIVector16& enumMembers, const CHIR::Type* boxTy)
{
    ...
    if (isRefArg) {
        ...
        auto refSizeInBits = CreateRefType()->getSizeInBits();
        auto elementType = CreatePointerType(argType, refSizeInBits);
        ...
        fwdDecl = createStructType(defPackage, name, diFile, position.GetBeginPos().line,
            refSizeInBits + boxOffset, argType->getAlignInBits(), llvm::DINode::FlagZero, nullptr,
            getOrCreateArray({}), 0u, nullptr, "$" + std::to_string(boxOffset) + "_" + enumTy.ToString());
        if (boxTy) {
            typeCache[boxTy] = llvm::TrackingMDRef(fwdDecl);
        }
        auto offsetBits = argTy->IsRawArray() ? refSizeInBits : 0u;
        auto valueType = createMemberType(fwdDecl, "val", diFile, 0u, refSizeInBits, 0u,
            offsetBits + boxOffset, llvm::DINode::FlagZero, elementType);
