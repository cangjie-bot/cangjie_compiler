...
llvm::Value* IRBuilder2::DoSimpleCast(const CGValue& srcValue, const CGType& destType) {
    ...
    if (srcTypeAddrspace != destTypeAddrspace) {
        return CreateAddrSpaceCast(&srcRawVal, &destLLVMType);
    } else {
        return CreateBitCast(&srcRawVal, &destLLVMType);
    }
}
...
llvm::Value* IRBuilder2::FixFuncArg(const CGValue& srcValue, const CGType& destType)
{
    ...
    return DoSimpleCast(srcValue, destType);
}

std::vector<llvm::Value*> IRBuilder2::FixFuncArgs(const CGFunctionType& calleeType, std::vector<CGValue*> args, bool isThisArgInStruct) {
    ...
    for (auto arg : args) {
        bool isThis = idx == 0 && isThisArgInStruct;
        auto& paramType = *calleeType.GetParamType(idx);
        auto llvmVal = isThis ?
            DoSimpleCast(*arg, paramType) :
            FixFuncArg(*arg, paramType);
...
llvm::Value* IRBuilder2::CreateSRetUnknownSize(CHIR::Type* returnCHIRType, const CHIRCallExpr &applyWrapper) {
    auto returnCGType = CGType::GetOrCreate(cgMod, returnCHIRType);

    llvm::Value* allocaForRetVal = nullptr;
    CHIR::Type* retValType = applyWrapper.GetResult()->GetType();
    CJC_ASSERT(retValType);
    ...
    std::vector<CGValue*> args, bool isClosureCall, llvm::Value* thisTypeInfo)
{
    ...
    // CHIR{Apply,Invoke,InvokeStatic}Expr
    auto applyWrapper = dynamic_cast<const CHIRCallExpr*>(this->chirExpr);

    std::vector<llvm::Value*> argsVal = FixFuncArgs(calleeType, args,
        applyWrapper && applyWrapper->IsCalleeStructInstanceMethod());
    ...
    if (calleeType.HasSRet()) {
        ...
        if (returnCGType->GetSize()) {
            ...
            allocaForRetVal = CreateSRetUnknownSize(returnCHIRType, *applyWrapper);
        }
        argsVal.insert(argsVal.begin(), allocaForRetVal);
    }

    llvm::PointerType *typeInfoPtrType{nullptr};
    auto getTypeInfoPtrType = [&typeInfoPtrType, this] () {
        if (!typeInfoPtrType) {
            typeInfoPtrType = CGType::GetOrCreateTypeInfoPtrType(cgMod.GetLLVMContext());
        }
        return typeInfoPtrType;
    };
    ...
    if (applyWrapper) {
        ...
        for (auto instantiateArg : applyWrapper->GetInstantiatedTypeArgs()) {
            ...
            typeInfo = CreateBitCast(typeInfo, getTypeInfoPtrType());
        ...
        if (applyWrapper->IsCalleeMethod()) {
            ...
            if (this->chirExpr->GetExprKind() == CHIR::ExprKind::APPLY ||
                ...
                // Apply
                typeInfo = CreateBitCast(CreateTypeInfo(outerCHIRType), getTypeInfoPtrType());
            } else {
                // Invoke[Static]
                ...
                if (applyWrapper->IsCalleeStatic()) {
                    ...
                    if (DeRef(*applyWrapper->GetThisType())->IsThis()) {
                        ...
                        typeInfo = CreateBitCast(CreateTypeInfo(applyWrapper->GetThisType()), getTypeInfoPtrType());
                    }
                    if (GetCGContext().GetCGPkgContext().NeedOuterTypeInfo(*outerCHIRClassType)) {
                        auto introType = CreateBitCast(
                            CreateTypeInfo(outerCHIRType), getTypeInfoPtrType());
                        ...
                        typeInfo = CreateBitCast(outerType, getTypeInfoPtrType());
                    }
                } else {
                    // Invoke
                    ...
                    if (!DeRef(*StaticCast<CHIRInvokeWrapper>(applyWrapper)->GetObject()->GetType())->IsAutoEnv() &&
                        ...
                        auto introType =
                            CreateBitCast(CreateTypeInfo(outerCHIRType), getTypeInfoPtrType());
                        auto outerType = CallIntrinsicMethodOuterType({typeInfo, introType,
        ...
        auto typeInfo = CreateBitCast(
            GetTypeInfoFromObject(autoEnvVal), getTypeInfoPtrType());
        (void)argsVal.emplace_back(typeInfo);
    }
