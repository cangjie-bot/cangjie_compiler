...
void IRBuilder2::EmitLocation(const CHIRExprWrapper& debugNode, const bool removeable)
{
    if (!cgMod.GetCGContext().IsLineInfoEnabled()) {
        return;
    }
    llvm::DILocation* diLoc = cgMod.diBuilder->CreateDILoc(debugNode, removeable);
