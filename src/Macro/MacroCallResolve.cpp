...
std::string GetMacroFuncInputType(const FuncDecl& funcDecl) {
    bool isAttrMacro = funcDecl.funcBody->paramLists.size() == 2;
    auto &type = funcDecl.funcBody->paramLists[0]->params[isAttrMacro ? 1 : 0]->type;
    return type->name;
}
}
...
bool MacroCall::GetValidFuncDecl(std::vector<Ptr<Decl>>& decls)
{
    ...
    return GetMatchFuncDecl(fds);
}

bool MacroCall::GetMatchFuncDecl(const std::vector<Ptr<FuncDecl>>& funcDecls)
{
...
bool MacroCall::BindInvokeFunc()
{
    ...
    if (node->TestAttr(AST::AttrKind::LATE_MACRO)) {
        inputTyName = invocation->hasParethesis ? MC_INPUT_EXPR : MC_INPUT_DECL;
    }
