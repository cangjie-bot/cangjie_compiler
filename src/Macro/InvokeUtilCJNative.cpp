...
bool RuntimeInit::InitRuntimeMethod()
{
    ...
    initLibFunc = reinterpret_cast<int(*)(const char*)>(InvokeRuntime::GetMethod(handle, G_CJ_LOAD_CJ_LIB.data()));
    if (!initLibFunc) {
        Errorln("could not find the load library method: ", G_CJ_LOAD_CJ_LIB);
        return false;
    }
    return true;
}
