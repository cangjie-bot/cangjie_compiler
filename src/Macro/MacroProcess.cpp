    ...
    std::map<Ptr<File>, std::vector<Ptr<Node>>>& macroCallMap, CompilerInstance& ci)
{
    ...
    for (const auto& file : fileSucVec) {
        ...
        // Pre-allocate string buffer to reduce reallocations
        std::string content;
        content.reserve(4096); // Reserve initial capacity
        ...
        for (auto& node : nodes) {
            ...
            // Cache source reference to avoid repeated GetSource calls in the loop
        ...
        if (ci.invocation.globalOptions.enableMacroInLSP || ci.invocation.globalOptions.enableMacroDebug) {
            // Get source after all AppendSource calls to get final buffer
            const auto& source = sm.GetSource(fileID);
            if (!WriteStringToFile(file->macroCallFilePath, source.buffer)) {
