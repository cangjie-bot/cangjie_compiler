...
std::string GetSystemErrorMessage(int error)
{
    ...
    if (stderror_r(error, buf, buffSize) != 0) {
        return "";
    }
    return std::string(buf);
#else
    return std::string(stderror_r(error, buf, buffSize));
#endif
}
#endif
}
...
std::unique_ptr<ToolFuture> Tool::Run() const
{
    ...
    if (status != 0) {
        Utils::Semaphore::Get().Release();
        return return std::make_unique<ErrorFuture>(GetSystemErrorMessage(status));;
    }
