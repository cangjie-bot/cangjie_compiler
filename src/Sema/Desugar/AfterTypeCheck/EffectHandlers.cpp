...
void TypeChecker::TypeCheckerImpl::EncloseTryLambda(ASTContext& ctx, OwnedPtr<AST::LambdaExpr>& tryLambda)
{
...

    {
        ...
        SynthesizeWithoutRecover(ctx, parentAccess.get(), SynthesizeContext::EXPR_ARG);
    ...
    for (auto& handler : te.handlers) {
        ...
        for (uint64_t j = 0; j < commandPattern->types.size(); j++) {
...

            {
                ...
                auto getActiveFrameCall = CreateCallExpr(std::move(getActiveFrame), {});
                SynthesizeWithoutRecover(ctx, getActiveFrameCall.get(), SynthesizeContext::EXPR_ARG);
            ...
            SynthesizeWithoutRecover(ctx, tryExpr, SynthesizeContext::NONE); // none for now
            ...
            SynthesizeWithoutRecover(ctx, resultBlock, SynthesizeContext::EXPR_ARG);
