...
Ptr<Ty> TypeManager::GetUnaliasedTypeFromTypeAlias(
    const TypeAliasTy& target, const std::vector<Ptr<Ty>>& typeArgs, bool needSubstituteGeneric)
{
    ...
    if (auto nestedAlias = DynamicCast<TypeAliasTy>(type)) {
...
Ptr<AST::Ty> TypeManager::SubstituteTypeAliasInTy(AST::Ty& ty, bool needSubstituteGeneric, const TypeSubst& typeMapping)
{
    ...
    switch (ty.kind) {
        ...
        case TypeKind::TYPE: {
            return GetUnaliasedTypeFromTypeAlias(static_cast<TypeAliasTy&>(ty), typeArgs, needSubstituteGeneric);
        }
        case TypeKind::TYPE_TUPLE: {
            return GetTupleTy(typeArgs);
        }
