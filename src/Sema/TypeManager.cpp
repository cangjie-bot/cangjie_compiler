    ...
    bool needSubstituteGeneric, const TypeSubst& customMapping)
{
    ...
    auto inner = GetInstantiatedTy(aliasedType->ty, typeMapping);
    if (auto nestedAlias = DynamicCast<TypeAliasTy>(inner)) {
        auto nestedAliasDecl = Ty::GetDeclPtrOfTy(nestedAlias);
        // the aliased type is in cycle, stop recursive substitution to avoid endless loop
        if (nestedAliasDecl && nestedAliasDecl->TestAttr(Attribute::IN_REFERENCE_CYCLE)) {
            return nestedAlias;
        }
        return SubstituteTypeAliasInTy(*nestedAlias, needSubstituteGeneric, customMapping);
    }
    return inner;
}
