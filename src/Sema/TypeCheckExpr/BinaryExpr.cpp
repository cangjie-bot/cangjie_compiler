...
bool TypeChecker::TypeCheckerImpl::ChkBinaryExpr(ASTContext& ctx, Ty& target, BinaryExpr& be)
{
    ...
    be.Clear();
    if (Ty::IsTyCorrect(SynthesizeWithNegCache(ctx, &be, SynthesizeContext::EXPR_ARG))) {
...
bool TypeChecker::TypeCheckerImpl::ChkExpoExprExponent(ASTContext& ctx, Expr& exponent, std::vector<Ptr<Ty>> exTys)
{
    ...
    if (answers > 1) {
        ...
        if (CanSkipDiag(exponent)) {
            (void)SynthesizeWithNegCache(ctx, &exponent, SynthesizeContext::EXPR_ARG); // Synthesize 'exponent' to report errors.
        } else {
            std::string tgtTyStr;
            if (exTys.size() == 1) {
...
{
    (void)SynthesizeWithNegCache(ctx, be.leftExpr.get(), SynthesizeContext::EXPR_ARG);
...
bool TypeChecker::TypeCheckerImpl::CheckExponentByBaseTy(ASTContext& ctx, Ty& baseTy, const Expr& base, Expr& exponent)
{
    ...
    if (baseTy.IsPlaceholder()) {
        ...
        SynthesizeWithNegCache(ctx, &exponent, SynthesizeContext::EXPR_ARG);
    ...
    if (isExponentWellTyped && typeManager.IsSubtype(&baseTy, i64)) {
        ...
        (void)SynthesizeWithNegCache(ctx, &exponent, SynthesizeContext::EXPR_ARG);
    }
    ...
    ASTContext& ctx, BinaryExpr& be, Ptr<Ty> target)
{
    ...
    if (Utils::In(be.op, ARITHMETIC_OPERATOR)) {
        ...
        bool bothAreTuple = Ty::IsTyCorrect(SynthesizeWithNegCache(ctx, be.leftExpr.get(), SynthesizeContext::EXPR_ARG)) &&
            be.leftExpr->ty->IsTuple() && Ty::IsTyCorrect(SynthesizeWithNegCache(ctx, be.rightExpr.get(), SynthesizeContext::EXPR_ARG)) &&
...
std::optional<Ptr<Ty>> TypeChecker::TypeCheckerImpl::SynLiteralInBinaryExprFromRight(ASTContext& ctx, BinaryExpr& be)
{
    ...
    {
        auto ds = DiagSuppressor(diag);
        bool isWellTyped = SynthesizeWithNegCache(ctx, be.rightExpr.get(), SynthesizeContext::EXPR_ARG) && ReplaceIdealTy(*be.rightExpr);
        if (!isWellTyped) {
            be.ty = TypeManager::GetInvalidTy();
            return {};
        }
    ...
    if (!isWellTyped) {
        ...
        SynthesizeWithNegCache(ctx, be.leftExpr.get(), SynthesizeContext::EXPR_ARG);
        be.ty = TypeManager::GetInvalidTy();
...
{
    bool isWellTyped = SynthesizeWithNegCache(ctx, be.leftExpr.get(), SynthesizeContext::EXPR_ARG) && ReplaceIdealTy(*be.leftExpr);
    ...
    if (!isWellTyped) {
        // Bind target when error happens for LSP.
        SynthesizeWithNegCache(ctx, be.rightExpr.get(), SynthesizeContext::EXPR_ARG);
        be.ty = TypeManager::GetInvalidTy();
        return TypeManager::GetInvalidTy();
    }
...
Ptr<Ty> TypeChecker::TypeCheckerImpl::SynLogicalExpr(ASTContext& ctx, BinaryExpr& be)
{
    CJC_NULLPTR_CHECK(be.leftExpr);
    CJC_NULLPTR_CHECK(be.rightExpr);
    auto leftTy = SynthesizeWithNegCache(ctx, be.leftExpr.get(), SynthesizeContext::EXPR_ARG);
    auto rightTy = SynthesizeWithNegCache(ctx, be.rightExpr.get(), SynthesizeContext::EXPR_ARG);
...
void TypeChecker::TypeCheckerImpl::DiagnoseForBinaryExpr(ASTContext& ctx, BinaryExpr& be)
{
    ...
    while (true) {
        ...
        if (Ty::IsInitialTy(pivot->leftExpr->ty)) {
            auto ds = DiagSuppressor(diag);
            SynthesizeWithNegCache(ctx, pivot->leftExpr, SynthesizeContext::EXPR_ARG); // in case the node was cleared and then skipped
        }
        if (!Ty::IsTyCorrect(pivot->leftExpr->ty)) {
            ...
            if (!Ty::IsTyCorrect(SynthesizeWithNegCache(ctx, pivot->leftExpr.get(), SynthesizeContext::EXPR_ARG))) {
                ...
                SynthesizeWithNegCache(ctx, pivot->rightExpr.get(), SynthesizeContext::EXPR_ARG);
                return;
            }
        }
        ...
        if (Ty::IsInitialTy(pivot->rightExpr->ty)) {
            auto ds = DiagSuppressor(diag);
            SynthesizeWithNegCache(ctx, pivot->rightExpr, SynthesizeContext::EXPR_ARG);
        }
        if (!Ty::IsTyCorrect(pivot->rightExpr->ty)) {
            ...
            if (!Ty::IsTyCorrect(SynthesizeWithNegCache(ctx, pivot->rightExpr.get(), SynthesizeContext::EXPR_ARG))) {
    ...
    while (pivotStack.size() > 1) {
        Ptr<BinaryExpr> pivot = pivotStack.back();
        pivotStack.pop_back();
        if (!Ty::IsTyCorrect(SynthesizeWithNegCache(ctx, pivot, SynthesizeContext::EXPR_ARG))) {
            return;
        }
    }
    Ptr<BinaryExpr> pivot = pivotStack.back();
    SynthesizeWithNegCache(ctx, pivot->leftExpr.get(), SynthesizeContext::EXPR_ARG);
    SynthesizeWithNegCache(ctx, pivot->rightExpr.get(), SynthesizeContext::EXPR_ARG);
...
Ptr<Ty> TypeChecker::TypeCheckerImpl::SynBinaryExpr(ASTContext& ctx, BinaryExpr& be)
{
    ...
    if (TypeCheckUtil::IsOverloadableOperator(be.op)) {
        DesugarOperatorOverloadExpr(ctx, be);
        auto ds = DiagSuppressor(diag);
        if (Ty::IsTyCorrect(Synthesize(ctx, be.desugarExpr.get(), SynthesizeContext::EXPR_ARG))) {
            ds.ReportDiag();
    ...
    ASTContext& ctx, BinaryExpr& be, Ptr<Ty>& inferRet)
{
    ...
    if (Utils::In(be.op, ARITHMETIC_OPERATOR)) {
        ...
        if (Ty::IsInitialTy(be.rightExpr->ty)) {
            (void)SynthesizeWithNegCache(ctx, be.rightExpr.get(), SynthesizeContext::EXPR_ARG);
        }
...
bool TypeChecker::TypeCheckerImpl::ChkFlowExpr(ASTContext& ctx, Ptr<Ty> target, BinaryExpr& be)
{
    ...
    {
        auto ds = DiagSuppressor(diag);
        (void)SynthesizeWithNegCache(ctx, &leftExpr, SynthesizeContext::EXPR_ARG);
        (void)SynthesizeWithNegCache(ctx, &rightExpr, SynthesizeContext::EXPR_ARG);
        if (ds.HasError()) {
