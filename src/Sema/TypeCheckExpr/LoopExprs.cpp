...
Ptr<Ty> TypeChecker::TypeCheckerImpl::SynWhileExpr(ASTContext& ctx, WhileExpr& we)
{
    ...
    isWellTyped = Ty::IsTyCorrect(Synthesize(ctx, we.body.get(), SynthesizeContext::UNUSED)) && isWellTyped;
    we.ty =
...
{
    bool isWellTyped = Ty::IsTyCorrect(Synthesize(ctx, dwe.body.get(), SynthesizeContext::UNUSED));
...
Ptr<Ty> TypeChecker::TypeCheckerImpl::SynForInExpr(ASTContext& ctx, ForInExpr& fie)
{
    ...
    bool isWellTyped = Synthesize(ctx, fie.inExpression.get(), SynthesizeContext::EXPR_ARG) && ReplaceIdealTy(*fie.inExpression);
    ...
    isWellTyped = Ty::IsTyCorrect(Synthesize(ctx, fie.body.get(), SynthesizeContext::UNUSED)) && isWellTyped;
