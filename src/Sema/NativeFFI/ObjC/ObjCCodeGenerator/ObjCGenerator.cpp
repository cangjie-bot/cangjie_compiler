...
{
    DecrementIndent();
    resSource += (newLineBefore ? "\n" : "") + std::string(indent, ' ') + "}" + (newLineAfter ? "\n" : "");
}
...
{
    buffer << std::string(indent, ' ') << Join("if (", cond, ")");
    WriteBlock(then);
    if (other) {
...
void ObjCGenerator::WriteBlock(std::function<void()> action, const std::string& pre, const std::string& suf, bool flush)
{
    ...
    action();
    DecrementIndent();
    buffer << std::string(indent, ' ') << Join("}", suf);
    if (flush) {
...
void ObjCGenerator::WriteSeq(const std::vector<std::string>& statements)
{
    for (auto& statement : statements) {
        buffer << std::string(indent, ' ') << statement << std::endl;
    }
}

void ObjCGenerator::IncrementIndent()
{
    indent += INDENT_SIZE;
}

void ObjCGenerator::DecrementIndent()
{
    indent -= INDENT_SIZE;
}
} // namespace Cangjie::Interop::ObjC
