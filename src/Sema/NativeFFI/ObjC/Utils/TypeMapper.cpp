...
bool TypeMapper::IsObjCCJMapping(const Decl& decl)
{
    ...
    return decl.TestAttr(Attribute::OBJ_C_CJ_MAPPING) && decl.generic == nullptr;
}
...
namespace {
bool SkipMemberKinds(const Decl& decl, const std::vector<ASTKind>& kinds) {
...
bool TypeMapper::IsObjCCJMappingMember(const AST::Decl& decl)
{
    ...
    if (outerDecl.astKind == ASTKind::CLASS_DECL) {
        if (!outerDecl.IsOpen() && SkipMemberKinds(decl, {ASTKind::PROP_DECL, ASTKind::VAR_DECL})) {
            return false;
        }
    }
    if (decl.astKind == ASTKind::FUNC_DECL) {
        if (auto fnTy = DynamicCast<FuncTy>(decl.ty)) {
            return IsValidCJMapping(*fnTy->retTy) && std::all_of(std::begin(fnTy->paramTys), std::end(fnTy->paramTys),
