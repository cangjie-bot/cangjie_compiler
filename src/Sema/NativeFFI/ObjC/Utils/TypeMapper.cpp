...
static constexpr auto OBJC_FUNC_TYPE = "ObjCFunc";
static constexpr auto OBJC_BLOCK_TYPE = "ObjCBlock";

template<class Type, class ToString>
std::string buildFunctionalCType(const std::vector<Type>& argTypes,
...
Ptr<Ty> TypeMapper::Cj2CType(Ptr<Ty> cjty) const
{
    ...
    if (IsObjCBlock(*cjty)) {
        return typeManager.GetPointerTy(
            bridge.GetObjCBlockAbiPointerAccessor()->ty->typeArgs.back()
        );
    }
...
bool IsObjCPointerImpl(const StructDecl& structDecl)
{
    ...
    if (structDecl.identifier != OBJC_POINTER_TYPE) {
        return false;
    }
    return true;
}
...
bool IsObjCFuncImpl(const StructDecl& structDecl)
{
    ...
    if (structDecl.identifier != OBJC_FUNC_TYPE) {
        return false;
    }
    return true;
}
...
{
    if (decl.fullPackageName != OBJC_LANG_PACKAGE) {
        return false;
    }
    if (decl.identifier != OBJC_BLOCK_TYPE) {
        return false;
    }
    return true;
}
