...
OwnedPtr<Expr> ASTFactory::CreateOptionalMethodGuard(OwnedPtr<Expr> msgSend, OwnedPtr<Expr> cls, const std::string& selector,
...
OwnedPtr<Expr> ASTFactory::CreateObjectGetClassCall(OwnedPtr<Expr> id, Ptr<File> curFile) {
    ...
    return WithinFile(CreateCallExpr(std::move(objectGetClassExpr), std::move(args), objectGetClassDecl,
        typeManager.GetBoolTy(), CallKind::CALL_DECLARED_FUNCTION), curFile);
}
