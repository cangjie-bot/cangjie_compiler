    ...
    const Native::FFI::GenericConfigInfo* genericConfig)
{
    ...
    auto wrapperName = nameGenerator.GenerateInitCjObjectName(ctor, &genericConfig->declInstName);
    ...
    const Native::FFI::GenericConfigInfo* genericConfig)
{
    ...
    auto funcName = nameGenerator.GenerateDeleteCjObjectName(target, &genericConfig->declInstName);
...
OwnedPtr<FuncDecl> ASTFactory::CreateMethodWrapper(FuncDecl& method, const Native::FFI::GenericConfigInfo* genericConfig)
{
    ...
    std::vector<Ptr<Ty>> funcTyParams;
    std::vector<OwnedPtr<Type>> actualPrimitiveType;
    Ptr<FuncTy> actualFuncTy;
    if (genericConfig && genericConfig->instTypes.size() != 0) {
        GetArgsAndRetGenericActualTyVector(method, genericConfig->instTypes, actualTyArgMap, funcTyParams, actualPrimitiveType);
    }
    ...
    if (retTy->HasGeneric()) {
        actualFuncTy = typeManager.GetFunctionTy(funcTyParams, typeMapper.Cj2CType(retActualTy), {.isC = true});
    }
    if (method.TestAttr(Attribute::STATIC)) {
        ...
        if (outerDecl->ty->HasGeneric()) {
            auto registryTy = CreateRefType(*outerDecl);
            registryTy->ty = instantTy;
    ...
    auto wrapperName = nameGenerator.GenerateMethodWrapperName(method, &genericConfig->declInstName);
