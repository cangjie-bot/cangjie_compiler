...
public:
    ...
    std::string GetJavaTypeSignature(const Ty& cjtype, Ptr<FuncDecl> fd = nullptr);
    std::string GetJavaTypeSignature(Ty& retTy, const std::vector<Ptr<Ty>>& args, Ptr<FuncDecl> fd = nullptr);
    std::string Utils::GetParamJavaSignature(const Ptr<Ty> ty, const Ptr<FuncDecl> fd = nullptr);
