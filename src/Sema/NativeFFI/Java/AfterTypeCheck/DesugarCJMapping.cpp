    ...
    std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>>* declActualMap)
{
    ...
    std::unordered_set<std::string> initGenericActualTypeSet;
                    ...
                    if (declIt != declActualMap->end()) {
                        ...
                        if (funcIt != funcDeclMap.end()) {
                            ...
                            for (auto& funcNameAndType : innerVector) {
                                ...
                                initGenericActualTypeSet.insert(funcNameAndType);
    ...
    if (declActualMap && initGenericActualTypeSet.size() == 0) {
        for (auto &declIt : *declActualMap) {
            auto& funcDeclMap = declIt.second;
            for (auto& funcIt : funcDeclMap) {
                auto& innerVector = funcIt.second;
                for (auto& funcNameAndType : innerVector) {
                    initGenericActualTypeSet.insert(funcNameAndType);
                }
    ...
    if (!generatedCtors.empty()) {
        ...
        for (auto generatedCtor : generatedCtors) {
            if (declActualMap) {
                for (auto& actualType : initGenericActualTypeSet) {
                    generatedDecls.push_back(GenerateNativeInitCjObjectFunc(*generatedCtor, false, false, nullptr, &actualType));
                }
            } else {
