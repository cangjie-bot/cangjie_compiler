...
void JavaDesugarManager::GenerateForCJStructOrClassTypeMapping(const File &file, AST::Decl* decl)
{
            ...
            if (fd->TestAttr(Attribute::CONSTRUCTOR)) {
                ...
                if (isGenericGlueCode) {
                    for (auto genericConfig : genericConfigsVector) {
                        auto nativeMethod = GenerateNativeMethod(*fd, *decl, &genericConfig->instTypes);
    ...
    if (!generatedCtors.empty()) {
        ...
        for (auto generatedCtor : generatedCtors) {
            if (isGenericGlueCode) {
                for (auto genericConfig : genericConfigsVector) {
                    generatedDecls.push_back(GenerateNativeInitCjObjectFunc(*generatedCtor, false, false, nullptr, &genericConfig->instTypes));
                }
            } else {
...
void JavaDesugarManager::GenerateForCJGenericTypeMapping(const File &file, AST::Decl* decl)
{
    ...
    if (classDecl || structDecl) {
        GenerateForCJStructOrClassTypeMapping(file, decl);
    }
}
