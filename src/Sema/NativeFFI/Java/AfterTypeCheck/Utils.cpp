 ...
 */
std::string Utils::GetJavaTypeSignature(const Ty& cjtype, const Ptr<FuncDecl> fd)
{
    std::string jsig;
    ...
    switch (cjtype.kind) {
        ...
        case TypeKind::TYPE_FUNC: {
            ...
            for (auto paramTy : funcTy.paramTys) {
                jsig.append(GetParamJavaSignature(paramTy, fd));
            }
            jsig.append(")");
            jsig.append(GetParamJavaSignature(funcTy.retTy, fd));
            break;
        }
...
std::string Utils::GetParamJavaSignature(const Ptr<Ty> ty, const Ptr<FuncDecl> fd)
{
    if (ty->kind == TypeKind::TYPE_FUNC) {
        CJC_NULLPTR_CHECK(fd);
        std::string javaClassName = GetLambdaJavaClassName(ty);
        return "L" + NormalizeJavaSignature(fd->fullPackageName + "." + javaClassName) + ";";
    } else {
        return GetJavaTypeSignature(*ty, fd);
    }
}

std::string Utils::GetJavaTypeSignature(Ty& retTy, const std::vector<Ptr<Ty>>& params, const Ptr<FuncDecl> fd)
{
    return GetJavaTypeSignature(*typeManager.GetFunctionTy(params, &retTy), fd);
}
