    ...
    Ptr<FuncDecl> fwdCtor, const std::string* actualType)
{
    ...
    if (isOpenClass) {
        ...
        auto actualPrimitiveType = GetOwnedPtrPrimitiveType(*actualType);
        auto instantiationRefType = CreateRefType(StaticCast<ClassDecl>(*ctor.outerDecl));
        instantiationRefType->typeArguments.emplace_back(std::move(actualPrimitiveType));
        // auto instantiationRefExpr = CreateRefExpr(ctor);
        // instantiationRefExpr->typeArguments.emplace_back(std::move(instantiationRefType));
        // auto ctorRef = WithinFile(std::move(instantiationRefExpr), curFile);
        objectCtorCall = CreateCallExpr(WithinFile(CreateRefExpr(ctor), curFile), std::move(ctorCallArgs), Ptr(&ctor), instantiationRefType->ty,
