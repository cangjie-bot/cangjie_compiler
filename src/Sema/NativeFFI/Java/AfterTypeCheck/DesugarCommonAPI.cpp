    ...
    if (actualTy) {
        ...
        if (sampleMethod.TestAttr(Attribute::STATIC)) {
            ...
            if (decl.ty->HasGeneric()) {
                ...
                // auto tmpTypeArgs = declty->typeArgs;
                // decl.ty->typeArgs.clear();
                // decl.ty->typeArgs.emplace_back(actualTy);
                // reg = lib.CreateGetFromRegistryCall(
                //     WithinFile(CreateRefExpr(jniEnvPtrParam), curFile), WithinFile(CreateRefExpr(selfParam), curFile),
                //     decl.ty);
                ...
                // Ptr<FuncTy> funcTy;
                // if (retTy->HasGeneric()) {
                //     funcTy = typeManager.GetFunctionTy(funcTyParams, actualTy);
                // } else {
                //     funcTy = typeManager.GetFunctionTy(funcTyParams, retTy);
                // }
                // methodAccess = CreateMemberAccess(std::move(reg), sampleMethod, funcTy);
