    ...
    std::vector<OwnedPtr<Type>> &actualPrimitiveType)
{
    ...
    for (size_t argIdx = 0; argIdx < ctor.funcBody->paramLists[0]->params.size(); ++argIdx) {
        ...
        if (arg->ty->IsGeneric()) {
            ...
            [&arg](const std::pair<std::string, std::string>& p) {
                return p.first == arg->ty->name;
            });
            if (it != instTypes.end()) {
                auto actualTypeKind = GetGenericActualTypeKind(it->second);
                Ptr<Ty> actualTy = TypeManager::GetPrimitiveTy(actualTypeKind);
                actualTy->name = arg->ty->name;
                actualTyArgMap[arg->ty->name] = actualTy;
    ...
    if (retTy->IsGeneric()) {
        auto retIt = std::find_if(instTypes.begin(), instTypes.end(),
        [&retTy](const std::pair<std::string, std::string>& p) {
            return p.first == retTy->name;
        });
        if (retIt != instTypes.end()) {
            auto actualTypeKind = GetGenericActualTypeKind(retIt->second);
            Ptr<Ty> actualRetTy = TypeManager::GetPrimitiveTy(actualTypeKind);
            actualRetTy->name = retTy->name;
            actualTyArgMap[retTy->name] = actualRetTy;
        }
    }
}
    ...
    std::vector<OwnedPtr<FuncArg>>& callArgs, FuncDecl& funcDecl, OwnedPtr<FuncParam>& arg, FuncParam& jniEnvPtrParam, Ptr<Ty> actualTy)
{
    ...
    Ptr<Ty> acutalArgTy = arg->ty->IsGeneric() ? actualTy : arg->ty;
    auto jniArgTy = GetJNITy(acutalArgTy);
    OwnedPtr<FuncParam> param = CreateFuncParam(arg->identifier.GetRawText(), nullptr, nullptr, jniArgTy);
    auto classLikeTy = DynamicCast<ClassLikeTy*>(acutalArgTy);
    ...
    if (IsMirror(*acutalArgTy)) {
        auto entity = lib.CreateJavaEntityJobjectCall(std::move(paramRef));
        methodArg = CreateFuncArg(CreateMirrorConstructorCall(importManager, std::move(entity), acutalArgTy));
    } else if (IsImpl(*acutalArgTy)) {
        auto entity = lib.CreateJavaEntityJobjectCall(std::move(paramRef));
        methodArg = CreateFuncArg(lib.UnwrapJavaEntity(std::move(entity), acutalArgTy, *outerDecl));
    } else if (acutalArgTy->IsCoreOptionType() && IsMirror(*acutalArgTy->typeArgs[0])) {
        // funcDecl(Java_CFFI_JavaEntity(arg)) // if arg is null (as jobject == 0) -> java entity will preserve it
        auto entity = lib.CreateJavaEntityJobjectCall(std::move(paramRef));
        methodArg = CreateFuncArg(lib.UnwrapJavaEntity(std::move(entity), acutalArgTy, *outerDecl));
    } else if (acutalArgTy->IsCoreOptionType() && IsImpl(*acutalArgTy->typeArgs[0])) {
        auto entity = lib.CreateJavaEntityJobjectCall(std::move(paramRef));
        methodArg = CreateFuncArg(lib.UnwrapJavaEntity(std::move(entity), acutalArgTy, *outerDecl));
    } else if (IsCJMapping(*acutalArgTy)) {
        auto entity = lib.CreateGetFromRegistryCall(
            WithinFile(CreateRefExpr(jniEnvPtrParam), funcDecl.curFile), std::move(paramRef), acutalArgTy);
        methodArg = CreateFuncArg(WithinFile(std::move(entity), funcDecl.curFile));
    } else if (IsCJMappingInterface(*acutalArgTy)) {
...
OwnedPtr<Decl> JavaDesugarManager::GenerateNativeMethod(FuncDecl& sampleMethod, Decl& decl, const std::vector<std::pair<std::string, std::string>> *instTypes)
{
    ...
    std::vector<OwnedPtr<Type>> actualPrimitiveType;
    if (instTypes) {
        getArgsAndRetGenericActualTyVector(sampleMethod, *instTypes, actualTyArgMap, funcTyParams, actualPrimitiveType);
    }
    ...
    std::string funcName = GetJniMethodName(sampleMethod, &actualTyArgMap);
    if (retTy->IsGeneric()) {
        funcName += '_';
        auto it = actualTyArgMap.find(retActualTy->name);
        if (it != actualTyArgMap.end()) {
            funcName += mangler.MangleType(*it->second);
        }
    }
...
OwnedPtr<Decl> JavaDesugarManager::GenerateNativeInitCjObjectFunc(FuncDecl& ctor, bool isClassLikeDecl, bool isOpenClass,
    Ptr<FuncDecl> fwdCtor, const std::vector<std::pair<std::string, std::string>> *instTypes)
{
    if (isClassLikeDecl) {
    ...
    std::vector<Ptr<Ty>> funcTyParams;
    std::vector<OwnedPtr<Type>> actualPrimitiveType;
    getArgsAndRetGenericActualTyVector(ctor, *instTypes, actualTyArgMap, funcTyParams, actualPrimitiveType);
    for (size_t argIdx = 0; argIdx < ctor.funcBody->paramLists[0]->params.size(); ++argIdx) {
        auto& arg = ctor.funcBody->paramLists[0]->params[argIdx];
    ...
    auto funcName = GetJniInitCjObjectFuncName(ctor, isClassLikeDecl, &actualTyArgMap);
