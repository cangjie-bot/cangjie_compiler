    ...
    if (ctor.outerDecl) {
        for (auto argTy : ctor.outerDecl->ty->typeArgs) {
            if (argTy->IsGeneric()) {
                ...
                [&argTy](const std::pair<std::string, std::string>& p) {
                    return p.first == argTy->name;
                });
                if (it != instTypes.end()) {
                    auto actualTypeKind = GetGenericActualTypeKind(it->second);
                    Ptr<Ty> actualTy = TypeManager::GetPrimitiveTy(actualTypeKind);
                    actualTy->name = argTy->name;
                    actualTyArgMap[argTy->name] = actualTy;
                }
            }
        }
    }
...
OwnedPtr<Decl> JavaDesugarManager::GenerateNativeMethod(FuncDecl& sampleMethod, Decl& decl, const GenericConfigInfo* genericConfig)
{
    ...
    std::string funcName = GetJniMethodName(sampleMethod, &genericConfig->declInstName);
    ...
    Ptr<FuncDecl> fwdCtor,  const GenericConfigInfo* genericConfig)
{
    ...
    auto funcName = GetJniInitCjObjectFuncName(ctor, isClassLikeDecl, &genericConfig->declInstName);
