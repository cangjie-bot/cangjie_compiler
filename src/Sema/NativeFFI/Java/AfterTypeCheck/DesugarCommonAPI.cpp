    ...
    std::string* actualType)
{
    ...
    if (actualTy) {
        ...
        bool argHasGeneric = false;
        auto instantTy = typeManager.GetClassTy(StaticCast<ClassDecl>(decl), {std::move(actualTy)});
        ...
        if (sampleMethod.TestAttr(Attribute::STATIC)) {
            ...
            if (decl.ty->HasGeneric()) {
                ...
                if (argHasGeneric && !retTy->IsGeneric()) {
                    methodAccess->ty = typeManager.GetFunctionTy({actualTy}, retTy);
                } else if (!argHasGeneric && retTy->IsGeneric()) {
                    methodAccess->ty = typeManager.GetFunctionTy({}, actualTy);
                } else if (argHasGeneric && retTy->IsGeneric()) {
                    methodAccess->ty = typeManager.GetFunctionTy({actualTy}, actualTy);
                }
            } else {
                reg = lib.CreateGetFromRegistryCall(
                    WithinFile(CreateRefExpr(jniEnvPtrParam), curFile), WithinFile(CreateRefExpr(selfParam), curFile),
        ...
        if (retTy->IsGeneric()) {
            ...
            if (methodCall->ty->HasGeneric()) {
                methodCall->ty = instantTy;
            }
    ...
    Ptr<FuncDecl> fwdCtor, const std::string* actualType)
{
    ...
    for (size_t argIdx = 0; argIdx < ctor.funcBody->paramLists[0]->params.size(); ++argIdx) {
        ...
        if (!FillMethodParamsByArg(params, ctorCallArgs, ctor, arg, jniEnvPtrParam, actualTy)) {
            return nullptr;
        }
    }
    ...
    if (isOpenClass) {
        ...
        Ptr<FuncTy> funcTy;
        if (retTy->HasGeneric()) {
            funcTy = typeManager.GetFunctionTy(funcTyParams, actualTy, {.isC = true});
        } else {
        ...
        auto callTy = typeManager.GetClassTy(*genericClassDecl, {actualTy});
