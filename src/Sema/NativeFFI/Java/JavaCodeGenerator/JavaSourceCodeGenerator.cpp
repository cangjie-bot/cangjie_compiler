            ...
            for (const auto& actualType : actualTypeSet) {
                ...
                switch (category) {
                    case TypeCategory::Integer:
                        ...
                        selfInitStream << TAB2 << "    self = initCJObjectInteger((Integer)" << genericParamName << ");\n";
                        break;
                    case TypeCategory::Long:
                        selfInitStream << "Long) {\n";
                        selfInitStream << TAB2 << "    self = initCJObjectLong((Long)" << genericParamName << ");\n";
                        break;
                    case TypeCategory::Float:
                        selfInitStream << "Float) {\n";
                        selfInitStream << TAB2 << "    self = initCJObjectFloat((Float)" << genericParamName << ");\n";
                        break;
                    case TypeCategory::Double:
                        selfInitStream << "Double) {\n";
                        selfInitStream << TAB2 << "    self = initCJObjectDouble((Double)" << genericParamName << ");\n";
                        break;
                    // others.
                    default:
                        break;
    ...
    std::string paramList, std::unordered_map<std::string, std::string> paramsMap, std::string actualType)
{
    ...
    if (retType != "void") {
        ...
        selfInitStream << TAB2 << mangledNativeName << "1" << actualType << "(";
    }
    ...
    if (!genericArgs.empty()) {
        ...
        selfInitStream << "this.self, " << A << genericArgs << ");\n";
    } else {
    ...
    if (retType != "void") {
        ...
        nativeMethodDefStream << TAB << "public native " << retType << " " << mangledNativeName << "1" << actualType << " (long self" <<
            comma << A << paramList << ");\n";
    }
...
void JavaSourceCodeGenerator::AddInstanceMethod(const FuncDecl& funcDecl, std::unordered_set<std::string> actualTypeSet)
{
    ...
    if (actualTypeSet.size() != 0) {
        ...
        for (const auto& actualType : actualTypeSet) {
            ...
            if (category == TypeCategory::Unknown) continue;
            if (firstBranch) {
                selfInstanceStream << "if (" << "t" << " instanceof ";
                firstBranch = false;
            } else {
                selfInstanceStream << TAB2 << "} else if (" << "t" << " instanceof ";
            }
            switch (category) {
                case TypeCategory::Integer: {
        ...
        // selfInstanceStream << TAB2 << "    throw new UnsupportOperationException(\"Unsupported type: \" + " << "t" <<
        //     ".getClass());\n";
    ...
    std::unordered_set<std::string> actualTypeSet)
{
    if (actualTypeSet.size() != 0) {
        for (auto& actualType : actualTypeSet) {
            ...
            if (actualType == "int32") {
                signature = "public native long " + initName + "(int v);\n";
            } else {
                signature = "public native long " + initName + "(" + strParams + ");\n";
            }
            AddWithIndent(TAB, signature);
        }
    } else {
