    ...
    TypeManager& tm, std::vector<OwnedPtr<FunctionMatchingUnit>>& candidates, std::vector<bool>& targetMark)
{
    for (size_t i = 0; i < candidates.size(); ++i) {
        ...
        for (size_t j = 0; j < candidates.size(); ++j) {
            ...
            if (!Ty::IsTyCorrect(instOuter2) || instOuter2 == instOuter1) {
                continue;
            }
            auto instFuncTy2 = StaticCast<FuncTy>(tm.ApplySubstPack(candidates[j]->fd.ty, candidates[j]->typeMapping));
            // Some generics case:
            // 1. 'func foo<T>(x: Rune, y: T)' and 'func foo<R, T>(x: R, y: T)' are not same signature.
            // 2. 'func foo<T>(x: Rune, y: T)' and 'func foo<T>(x: T, y: Rune)' are not same signature.
            // 3. 'func foo<T>(x: Rune, y: T)' and 'func foo<T>(x: Rune, y: T)' are same signature.
            auto& funcBody2 = candidates[j]->fd.funcBody;
            bool sameSig = instFuncTy1 == instFuncTy2;
            ...
            if (tm.IsSubtype(instOuter2, instOuter1) && sameSig) {
                targetMark[i] = false;
                break;
            }
        }
