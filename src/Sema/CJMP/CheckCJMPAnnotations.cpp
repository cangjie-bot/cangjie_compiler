...
static const std::unordered_map<AST::AnnotationKind, std::string> ANNO_KIND_TO_NAME = [] {
...
static Ptr<Annotation> GetDeprecated(const AST::Decl& node)
{
...
static bool PostCheckDeprecatedAnnotation(const AST::Decl& common, const AST::Decl& platform, DiagnosticEngine& diag)
{
    (void)common;
    auto platformDeprecation = GetDeprecated(platform);
...
{
    if (a.name != b.name) {
        // named args are not yet supported but we check it if positions matched
        return false;
    }
...
static bool AnnotationEquals(const AST::Annotation& a, const AST::Annotation& b)
{
    ...
    for (size_t i = 0; i < a.args.size(); ++i) {
        if (!FuncArgEquals(*a.args[i], *b.args[i])) {
            return false; // named args are not yet supported
        }
    }
...
static bool SetContains(const std::unordered_set<Ptr<const AST::Annotation>>& set, Ptr<const AST::Annotation> anno)
{
    return set.find(anno) != set.end();
}
    ...
    const std::unordered_set<Ptr<const AST::Annotation>>& excludes)
{
    auto found = std::find_if(decl.annotations.begin(), decl.annotations.end(), [&sample, &excludes](const auto& anno) {
        return !SetContains(excludes, anno.get()) && AnnotationEquals(sample, *anno.get());
    });
...
static Ptr<const AST::Annotation> FindFirstAnnotationByKind(const AST::Decl& decl, AnnotationKind kind)
{
...
static std::string AnnotationKindToString(AnnotationKind kind)
{
    ...
    const AST::Decl& common, const AST::Decl& platform, DiagnosticEngine& diag)
{
    ...
    for (const auto& [annoKind, attr] : NonSerializedAnnotations) {
        if (common.TestAttr(attr) != platform.TestAttr(attr)) {
            ...
            auto platformAnno = FindFirstAnnotationByKind(platform, annoKind);
    ...
    std::unordered_set<Ptr<const AST::Annotation>>& matchedTarget, DiagnosticEngine& diag)
{
    ...
    for (auto& anno : source.annotations) {
        if (IsSpecialHandledAnnotation(*anno) || SetContains(matchedSource, anno.get())) {
            continue;
        }
...
{
    bool result = PostCheckDeprecatedAnnotation(common, platform, diag);
...
{
    Ptr<Annotation> commonDeprecation = GetDeprecated(common);
    Ptr<Annotation> platformDeprecation = GetDeprecated(platform);
