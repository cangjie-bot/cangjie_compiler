...
void PluginCustomAnnoChecker::Parse(const Decl& decl, PluginCustomAnnoInfo& annoInfo)
{
    ...
    if (auto fd = DynamicCast<FuncDecl>(&decl); fd && fd->outerDecl) {
        CJC_NULLPTR_CHECK(ci.typeManager);
        PluginCustomAnnoInfo outerAnnoInfo;
        Parse(*fd->outerDecl, outerAnnoInfo);
        bool hideOrInHide = hideExist || outerAnnoInfo.hasHideAnno;
        auto overridedFd = ci.typeManager->GetTopOverriddenFuncDecl(fd);
        if (overridedFd && hideOrInHide) {
            PluginCustomAnnoInfo overridedAnnoInfo;
            Parse(*overridedFd, overridedAnnoInfo);
            if (!overridedAnnoInfo.hasHideAnno) {
                diag.DiagnoseRefactor(DiagKindRefactor::sema_hide_missing_hide, *overridedFd);
            }
        }
    }
