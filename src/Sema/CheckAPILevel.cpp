...
inline std::string_view GetModuleName(const std::string_view& fullPackageName)
{
...
void PluginCustomAnnoChecker::CheckHideOfOverrideFunction(const Decl& decl, const PluginCustomAnnoInfo& annoInfo)
{
    ...
    if (functionWithHide.has_value() && !overriddenFdWithHide.has_value()) {
        // @!Hide func f(): Unit {}; override func f(): Unit {} -> error
        ...
        auto builder = diag.DiagnoseRefactor(DiagKindRefactor::sema_hide_missing_hide, *fd);
        builder.AddNote(*overriddenFd, "the overridden function is marked with '@!Hide'");
