...
void PrintBasic(unsigned indent, const Node& node, std::ostream& stream = std::cout)
{
    ...
    if (auto d = DynamicCast<Decl>(&node)) {
        PrintIndent(stream, indent, "linkage:", static_cast<int>(d->linkage), ", isConst:", static_cast<int>(d->IsConst()));
...
void PrintVarWithPatternDecl(unsigned indent, const VarWithPatternDecl& varWithPatternDecl, std::ostream& stream = std::cout)
{
...
void PrintBinaryExpr(unsigned indent, const BinaryExpr& binaryExpr, std::ostream& stream = std::cout)
{
    ...
    if (binaryExpr.overflowStrategy != OverflowStrategy::NA) {
        PrintIndent(stream, indent + ONE_INDENT, "overflowStrategy:", OverflowStrategyName(binaryExpr.overflowStrategy));
    }
    PrintIndent(stream, indent, "}");
}
...
void PrintOptionalChainExpr(unsigned indent, const OptionalChainExpr& optionalChainExpr, std::ostream& stream = std::cout)
{
...
void PrintCommandTypePattern(unsigned indent, const CommandTypePattern& commandTypePattern, std::ostream& stream = std::cout)
{
    ...
    unsigned indent, const FeaturesDirective& featuresDirective, std::ostream& stream = std::cout)
{
    ...
    PrintIndent(stream, indent + ONE_INDENT, "]");
    PrintIndent(stream, indent + ONE_INDENT, "position:", featuresDirective.begin.ToString(), featuresDirective.end.ToString());
    PrintIndent(stream, indent, "}");
}
...
void PrintNode([[maybe_unused]] Ptr<const Node> node, [[maybe_unused]] unsigned indent,
    [[maybe_unused]] const std::string addition, std::ostream& stream)
{
    if (!addition.empty()) {
