...
def generate_cmake_defs(args):
    ...
    if args.target:
        assert(args.target_toolchain , "When cross-compiling target-toolchain can not be empty")
        cross_c_compiler = shutil.which(args.target + "-gcc", path=args.target_toolchain)
        cross_cxx_compiler = shutil.which(args.target + "-g++", path=args.target_toolchain)
